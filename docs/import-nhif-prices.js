/**
 * Google Apps Script за импортиране на НЗОК цени
 * 
 * ИНСТРУКЦИИ:
 * 1. Отвори Google Sheet базата данни
 * 2. Extensions -> Apps Script
 * 3. Добави този код като нов файл или добави функцията към съществуващия
 * 4. Изпълни функцията: importNHIFPrices()
 */

/**
 * Импортира реалните НЗОК цени в листа NHIF_Prices
 * Всички цени са в ЕВРО
 */
function importNHIFPrices() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName('NHIF_Prices');
  
  if (!sheet) {
    sheet = ss.insertSheet('NHIF_Prices');
  }
  
  // Изчисти старите данни
  sheet.clear();
  
  // НЗОК Ценоразпис - реални данни в ЕВРО
  const nhifPrices = [
    // Прегледи
    ['nhif_101', '101', 'Обстоен преглед със снемане на орален статус', 16.77, 16.77, 0, 0, 'Преглед'],
    ['nhif_101b', '101', 'Обстоен преглед - бременни', 16.77, 0, 0, 0, 'Преглед'],
    
    // Обтурации (пломби)
    ['nhif_301', '301', 'Обтурация с химичен композит', 45.77, 43.73, 0, 2.05, 'Лечение'],
    
    // Екстракции
    ['nhif_508', '508', 'Екстракция на временен зъб с анестезия', 18.39, 0, 0, 0, 'Хирургия'],
    ['nhif_509', '509', 'Екстракция на постоянен зъб с анестезия', 45.77, 43.73, 0, 2.05, 'Хирургия'],
    
    // Ендодонтия
    ['nhif_332', '332', 'Лечение на пулпит/периодонтит на временен зъб', 24.63, 0, 2.40, 0, 'Ендодонтия'],
    ['nhif_333', '333', 'Лечение на пулпит/периодонтит на постоянен зъб', 79.42, 0, 3.08, 0, 'Ендодонтия'],
    
    // Протези
    ['nhif_832', '832', 'Горна цяла плакова зъбна протеза (клинична дейност)', 147.19, 0, 0, 0, 'Протетика'],
    ['nhif_833', '833', 'Долна цяла плакова зъбна протеза (клинична дейност)', 147.19, 0, 0, 0, 'Протетика'],
    ['nhif_834', '834', 'Техническа изработка горна протеза (лаборатория)', 31.12, 0, 0, 0, 'Протетика'],
    ['nhif_835', '835', 'Техническа изработка долна протеза (лаборатория)', 31.12, 0, 0, 0, 'Протетика']
  ];
  
  // Headers
  const headers = [
    'id',
    'code', 
    'name',
    'priceUnder18',
    'priceOver18', 
    'patientPayUnder18',
    'patientPayOver18',
    'category'
  ];
  
  // Запиши заглавния ред
  sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
  
  // Форматиране на заглавния ред
  sheet.getRange(1, 1, 1, headers.length)
    .setBackground('#ec4899')
    .setFontColor('#ffffff')
    .setFontWeight('bold');
  
  // Добави данните
  sheet.getRange(2, 1, nhifPrices.length, headers.length).setValues(nhifPrices);
  
  // Форматиране на цените в EUR
  sheet.getRange('D2:G1000').setNumberFormat('#,##0.00 "€"');
  
  // Разшири колоните
  sheet.setColumnWidth(1, 80);  // id
  sheet.setColumnWidth(2, 50);  // code
  sheet.setColumnWidth(3, 400); // name
  sheet.setColumnWidth(4, 100); // priceUnder18
  sheet.setColumnWidth(5, 100); // priceOver18
  sheet.setColumnWidth(6, 120); // patientPayUnder18
  sheet.setColumnWidth(7, 120); // patientPayOver18
  sheet.setColumnWidth(8, 100); // category
  
  sheet.setFrozenRows(1);
  
  SpreadsheetApp.getUi().alert('✅ Импортирани са ' + nhifPrices.length + ' НЗОК процедури в ЕВРО!');
}

/**
 * Добавя бутон в менюто за импортиране
 */
function onOpen() {
  const ui = SpreadsheetApp.getUi();
  ui.createMenu('🦷 Родопи Дент')
    .addItem('Настрой базата данни', 'setupDatabase')
    .addItem('Импортирай НЗОК цени', 'importNHIFPrices')
    .addSeparator()
    .addItem('Добави тестови записи', 'addTestAppointments')
    .addToUi();
}
